...
import user from '../../../assets/auth/user.png'; // Default fallback image
import { useGetAllUsersQuery, useGetsingleUserQuery } from '../../../redux/features/user/userApi';

const imageApi = "http://10.0.60.18:9090/"; // Base URL for images

const UserDetailsPage = () => {
    const [selectedUser, setSelectedUser] = useState(null);

    // Fetch all users for the left side
    const { data: allUser, isFetching, isError, error } = useGetAllUsersQuery();

    // Fetch details of the selected user
    const { data: singleUser, isFetching: isSingleUserFetching, isError: isSingleUserError, error: singleUserError } = useGetsingleUserQuery(selectedUser ? { id: selectedUser } : null, {
    ...
    // Handle loading and error states for single user
    if (isSingleUserFetching) {
        return <div>Loading user details...</div>;
    }

    if (isSingleUserError) {
        return <div>Error: {singleUserError?.message}</div>;
    }

    const users = allUser?.data?.attributes?.results || [];

    // Default to the first user if none is selected
    const userDetails = singleUser || users.find(user => user.id === selectedUser) || users[0];

    // Function to get the full URL for the user's profile image
    const getProfileImageUrl = (profileImage) => {
        return profileImage ? `${imageApi}${profileImage}` : user; // Use profile image or fallback
    };
    ...
    return (
        <div className="flex p-6 space-x-6">
                            ...
                            >
                                ...
                                <td className="px-4 py-2">
                                    <div className="flex items-center">
                                        <img
                                            className="w-10 h-10 rounded-full mr-2"
                                            src={getProfileImageUrl(user.profileImage)} // Get full URL for profile image
                                            alt={user.firstName}
                                        />
            ...
            <div className="flex-1 bg-white rounded-lg shadow-lg p-6">
                ...
                <div className="flex items-center mb-4">
                    <img
                        src={getProfileImageUrl(userDetails.profileImage)} // Get full URL for profile image
                        alt="User Profile"
                        className="w-24 h-24 rounded-full mr-4"
                    />
                    <div>
                        <h3 className="font-semibold text-lg text-gray-800">{userDetails.firstName} {userDetails.lastName}</h3>
                        <p className="text-sm text-gray-500">Joined on {userDetails.createdAt ? new Date(userDetails.createdAt).toLocaleDateString() : 'N/A'}</p>
                    </div>
                </div>
